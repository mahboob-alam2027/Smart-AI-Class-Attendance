...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning("You have not created any subjects yet! create one to begin!")
        return
    ...
    with col1:
        selected_student_label = st.selectbox('Select Subject', options =list(subject_options.keys()))

    with col2:
        if st.button("Add Photos", type='primary', icon=':material/photo_prints:', width='stretch'):
            add_photos_dialog()

    selected_subject_id = subject_options[selected_student_label]
...
def teacher_screen_login():
    ...
    with btnc1:
        if st.button('Login  ⌘ + Enter',  icon=':material/passkey:', use_container_width=True):
...
def teacher_screen_register():
    ...
    with btnc1:
        if st.button('Register now  ⌘ + Enter', icon=':material/passkey:', use_container_width=True):
